...
export function printAlgebraicType(
  ...
  if (type.Array) {
    ...
    return "Unhandled array, please report this";
  }
  ...
  if (type.Sum) {
    ...
    return "Unhandled sum,please report this";
  }
  if (type.Ref) {
  ...
  if (type.Product) {
    ...
    return "Unhandled product, please report this";
  }

  return Object.keys(type)[0].toLocaleLowerCase();
}

export function getReducerLifecycle(reducer: Reducer): string | undefined {
  if (reducer.lifecycle.some) {
    return Object.keys(reducer.lifecycle.some)[0];
  }
  return undefined;
}
